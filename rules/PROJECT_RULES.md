...
- The source release must carry an explicit MIT LICENSE and Fastlane metadata versioned with the tagged release source.
...
- CapsViewer remains a comparison floor for relevant OpenGL ES/EGL capability presentation, while Khronos core/extension specifications are authoritative when the two differ or the registry gains newer enumerants.
- Release metadata in README, Gradle, release notes, changelog, Fastlane metadata and audit tooling must agree on versionName/versionCode.
